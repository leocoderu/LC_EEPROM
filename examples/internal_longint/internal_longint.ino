...
void setup(){
  ...
  defV = eeprom.intReadLong(ADDR);              // Save default value
  ...
  //res = eeprom.intWriteLong(ADDR, 0xFFFFFFFF); 
  ...
  // Get value by address
  gLong = eeprom.intReadLong(ADDR);     
  //Serial.print("Now value by address 0x"); Serial.print(ADDR, HEX); Serial.print(" is: \t0x"); Serial.println(gLong, HEX);
  Serial.print("EEPROM value now \t0x"); Serial.println(gLong, HEX);

  //eeprom.intShow(ADDR, 0x041F, 32);
}
