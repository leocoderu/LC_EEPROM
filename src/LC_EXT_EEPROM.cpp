...
uint8_t LC_EXT_EEPROM::extRead(const uint32_t& addr, char* dst, const uint8_t& szDst, const bool endZero) {
    if ((addr + szDst - 1) >= _totalCapacity) return 1; // If we try read block outside, return error
    memset(dst, 0x00, szDst);                           // Set destination array values by default values
    for (uint8_t i = 0; i < szDst; i++) {
        char v = '\0';
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const uint16_t& src) {
    ...
    if (z != src) {                                             // Check data in memory and src 
        uint8_t wData[sz] = {};
        for (uint8_t n = 0; n < sz; n++) 
            wData[n] = (uint8_t)(src >> (8 * (sz - 1 - n)));    // Build array of data
        if (_write(addr, wData, sz) != 0) return 2;             // Write data to memory
        if (extRead(addr, z) != 0) return 2;                    // Get wrote data
        if (z != src) return 2;                                 // Check wrote data and src
    }
    return 0;
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const float& src) {
    ...
    if (z != v) {                                       // Check data in memory and src 
        if (extWrite(addr, v) != 0) return 2;           // Write unsigned data
        if (extRead(addr, z) != 0) return 2;            // Get wrote data
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const uint8_t& nBit, const bool& state) {
    if ((nBit > 7) || (addr >= _totalCapacity)) return 1;       // Check outmemory
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const String& src) {
    if ((addr + src.length() - 1) >= _totalCapacity) return 1;  // Check outmemory

    String z = "";
    if (extRead(addr, z, src.length()) != 0) return 2;          // Get data from memory
    if (z != src) {                                             // Check data in memory and src 
        uint8_t wData[src.length()];
        for (uint8_t i = 0; i < src.length(); i++) 
            wData[i] = (uint8_t)src[i];                         // Build array of data      
        if (_write(addr, wData, src.length()) != 0) return 2;   // Write data to memory
        if (extRead(addr, z, src.length()) != 0) return 2;      // Get data from memory
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const uint8_t* src, const uint8_t& szSrc) {
    if ((addr + szSrc - 1) >= _totalCapacity) return 1;     // Check outmemory

    uint8_t buff[szSrc] = {};
    if (extRead(addr, buff, szSrc) != 0) return 2;          // Get data from memory
    if (!_cmpBuffers(src, szSrc, buff)) {                   // Check data in memory and src 
        ...
        for (uint8_t i = 0; i < szSrc; i++)
            if (extWrite(addr + i, src[i]) != 0) return 2;  // Write data to memory
        if (extRead(addr, buff, szSrc) != 0) return 2;      // Get data from memory         
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const char* src, const uint8_t& szSrc, const bool endZero) {
    if ((addr + szSrc - 1) >= _totalCapacity) return 1;                     // Check outmemory     
    
    char buff[szSrc] = {};
    if (extRead(addr, buff, szSrc, endZero) != 0) return 2;                 // Get data from memory
    if (!_cmpBuffers(src, szSrc, buff)) {                                   // Check data in memory and src 
        if (extFill(addr, szSrc, 0x00) != 0) return 2;                      // Clear value EEPROM by default value
        for (uint8_t i = 0; i < szSrc; i++) {            
            if (extWrite(addr + i, (uint8_t)(src[i] + 128)) != 0) return 2; // if src = -128 then res = 0, if src = +127 then res = 255        
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const uint16_t* src, const uint8_t& szSrc) {
    uint8_t sz = sizeof(uint16_t);                                  // Get size element of array
    if ((addr + (szSrc * sz) - 1) >= _totalCapacity) return 1;      // Check outmemory    
    ...
    if (!_cmpBuffers(src, szSrc, buff)) {                    // Check data in memory and src
        ...
        for (uint8_t i = 0; i < szSrc; i++)
            if (extWrite(addr + (i * sz), src[i]) != 0) return 2;   // Write data to memory
        if (extRead(addr, buff, szSrc) != 0) return 2;              // Get data from memory
        if (!_cmpBuffers(src, szSrc, buff)) return 2;        // Check data in memory and src
    }
    return 0;
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const int16_t* src, const uint8_t& szSrc) {
    ...
    if (!_cmpBuffers(src, szSrc, buff)) {                                        // Check data in memory and src
        ...
        for (uint8_t i = 0; i < szSrc; i++)
            if (extWrite(addr + (i * sz), (uint16_t)(src[i] + 32768)) != 0) return 2;   // Write data to memory
        if (extRead(addr, buff, szSrc) != 0) return 2;                                  // Get data from memory
        if (!_cmpBuffers(src, szSrc, buff)) return 2;                            // Check data in memory and src
    }
    return 0;
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const int32_t* src, const uint8_t& szSrc) {
    ...
    if (!_cmpBuffers(src, szSrc, buff)) {                                            // Check data in memory and src
        if (extFill(addr, szSrc * sz, 0x00) != 0) return 2;                                 // Clear value EEPROM by default value
        for (uint8_t i = 0; i < szSrc; i++)
            if (extWrite(addr + (i * sz), (uint32_t)(src[i] + 2147483648)) != 0) return 2;  // Write data to memory
        if (extRead(addr, buff, szSrc) != 0) return 2;                                      // Get data from memory
        if (!_cmpBuffers(src, szSrc, buff)) return 2;                                // Check data in memory and src
    }
...
uint8_t LC_EXT_EEPROM::extWrite(const uint32_t& addr, const int64_t* src, const uint8_t& szSrc) {
    ...
    if (!_cmpBuffers(src, szSrc, buff)) {                    // Check data in memory and src
        if (extFill(addr, szSrc * sz, 0x00) != 0) return 2;         // Clear value EEPROM by default value
        for (uint8_t i = 0; i < szSrc; i++)
            if (extWrite(addr + (i * sz), (uint64_t)(src[i] + 9223372036854775808UL)) != 0) return 2;   // Write data to memory
        if (extRead(addr, buff, szSrc) != 0) return 2;              // Get data from memory
        if (!_cmpBuffers(src, szSrc, buff)) return 2;        // Check data in memory and src
    }
    return 0;
